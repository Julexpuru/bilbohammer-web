...
export default function NotificationsMenu({ className }: Props) {
  ...
  const loadPushState = useCallback(async () => {
    ...
    const response = await fetch("/api/me/push-subscription", { cache: "no-store" });
  ...
  return (
    <div ref={wrapperRef} className={`relative ${className ?? ""}`}>
      ...
      {open && (
        <div className="absolute right-0 z-50 mt-2 w-[min(92vw,24rem)] rounded-2xl border border-[var(--hairline)] bg-[var(--card)] shadow-2xl">
          ...
          {settingsOpen && preferences && (
            <div className="border-b border-[var(--hairline)] bg-[var(--bg)] px-4 py-3">
