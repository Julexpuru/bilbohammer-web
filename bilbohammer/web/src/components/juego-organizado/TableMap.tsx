    ...
    <div className="flex flex-col gap-6">
            ...
            {tables.map((table) => {
              ...
              return (
                <div
                  key={table.id}
                  className={clsx(
                    "absolute rounded-xl border bg-white/80 px-2 py-1 text-xs font-semibold text-slate-900 shadow-lg backdrop-blur-sm transition ring-offset-2 ring-offset-[#f5f0e6]",
                ...
                >
                  <div className="flex items-center justify-between gap-1">
                    ...
                    {!isZone && (
                      <span className="rounded-full bg-white/80 px-2 py-[2px] text-[10px] uppercase tracking-wide text-slate-900 shadow-sm">
                        {stateLabel[state!]}
                      </span>
                    )}
                  </div>
                  {table.sizeTag && <div className="mt-1 text-[10px] font-normal text-slate-700">{table.sizeTag}</div>}
                </div>
              );
            })}
            {editing && canManage && (
      ...
      <div className="grid gap-4 lg:grid-cols-3">
        ...
        <div className="rounded-2xl border border-[var(--hairline)] bg-[var(--card)] p-4">
          ...
          {selectedTable && draft && canManage && editing ? (
            <div className="mt-3 space-y-3 text-sm text-[var(--muted)]">
              <div className="text-base font-semibold text-[var(--text)]">{selectedTable.name}</div>
              ...
              {tables.map((table) => {
                ...
                return (
                  <div
                    key={table.id}
                    className={clsx(
                      "absolute rounded-xl border bg-white/80 px-2 py-1 text-[10px] font-semibold text-slate-900 shadow-sm backdrop-blur-sm transition",
                      isZone ? "z-0" : "z-10",
                      hidden && "hidden"
                    )}
                    style={{
                  ...
                  >
                    <div className="flex items-center justify-between gap-1">
                      <span>{table.name}</span>
                      {!isZone && (
                        <span className="rounded-full bg-white/80 px-2 py-[2px] text-[9px] uppercase tracking-wide text-slate-900 shadow-sm">
                          {stateLabel[state!]}
                        </span>
                      )}
                    </div>
                    {table.sizeTag && <div className="mt-1 text-[9px] font-normal text-slate-700">{table.sizeTag}</div>}
                  </div>
                );
              })}
            </div>
