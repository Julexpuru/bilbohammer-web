...
export default async function TablonSociosPage() {
  ...
  if (!authorized) {
    return (
      <section className="card space-y-4">
        ...
        <p>
          Tu cuenta no dispone del rol de socio. Si crees que es un error, ponte en contacto con la junta para
          revisar tu alta.
        </p>
      </section>
    );
  ...
  const boardCards = boardMembers
    .map(toMemberCard)
    .sort((a, b) => a.displayName.localeCompare(b.displayName, "es"));
  const socioCards = socios
    .map(toMemberCard)
    .sort((a, b) => a.displayName.localeCompare(b.displayName, "es"));
  ...
  return (
    <div className="space-y-10">
      <section className="card space-y-3">
        <h1 className="text-3xl font-semibold">Tablon de socios</h1>
        <p>
          Este directorio interno te ayuda a contactar con la junta, encontrar personas para organizar eventos y
          poner cara a quienes comparten el local contigo.
        </p>
...
function Avatar({
  ...
  return (
    <div className={`${frame} ${ring}`}>
      <Image src={avatarUrl} alt={`Avatar de ${initials}`} fill className="object-cover" />
    </div>
  );
}
