...
const APPEARANCE_STYLES: Record<NonNullable<Props["appearance"]>, string> = {
  light: "border-[var(--hairline)] bg-[var(--card)] text-[var(--muted)] hover:text-[var(--text)] focus-visible:outline-[var(--accent-400)]",
  muted: "border-white/30 bg-white/10 text-white hover:bg-white/20 focus-visible:outline-white",
};
...
export function ArticleShareButtons({
  ...
  const relativePath = `/novedades/${category}/${slug}`;
  ...
  const whatsappUrl = useMemo(() => {
    ...
    const pieces = [title];
    if (summary) pieces.push(summary);
    pieces.push(shareUrl);
    return `https://wa.me/?text=${encodeURIComponent(pieces.join("\n\n"))}`;
  }, [title, summary, shareUrl]);
  ...
  const handleInstagramShare = useCallback(async () => {
    ...
    if (navigator.share) {
      try {
        ...
        // usuario canceló; continuamos con fallback
      }
    }
